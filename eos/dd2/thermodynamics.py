...
def kinetic_thermo(mu_eff, m, g, T=0.0):
    ...
    Returns (n, P, eps, s, n_s) in natural units. Both branches come from
    `eos.general.fermi_integrals`, the single home CLAUDE.md section 7 requires:
    the exact closed forms at T = 0, the Johns-Ellis-Lattimer integrals above
    it, converted from their fm-based units here so that no call site has to.

    `include_antiparticles=False` at T = 0 is DD2's convention that a mode with
    mu_eff < m is simply absent rather than filled by antiparticles. The
    massless case (neutrinos) needs no branch of its own: m = 0 kills the log
    and the m^2, m^4 terms of the closed forms and leaves n = g mu^3 / 6 pi^2,
    eps = g mu^4 / 8 pi^2, P = eps / 3, n_s = 0. It must NOT be routed through
    `solve_fermi_jel`, whose m < 1e-5 branch is tested before its T < 1e-4
    branch and returns the net particle-antiparticle density, negative where
    mu_eff < 0.
    """
    from eos.general.fermi_integrals import solve_fermi_jel, solve_fermi_t0
    if T == 0.0:
