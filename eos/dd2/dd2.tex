...
of one pin remains available as an option, though the third finite difference
behind it makes that closure unusable in practice (below).
...
predicts rather than imposes, are finite differences of a solved quantity;
the forward and inverse maps use the identical $h = 10^{-4}$ stencil so the
bias cancels exactly on a round trip, and any change to $h$ must be made in
both places together. Second, the isoscalar residual surface has spurious basins;
...
Fourth, $Q_{\mathrm{sat}}$ cannot be imposed while it is a third difference.
That closure conditions at $259$ and the $Q_{\mathrm{sat}}$ row carries a
relative floor near $1.5\times 10^{-3}$ --- the value spans $2.48$~MeV over
$h \in [5\times 10^{-5}, 5\times 10^{-4}]$, against $5.2\times 10^{-4}$~MeV
for $K_{\mathrm{sat}}$ --- so a solve inherits $O(0.4)$ of relative coupling
error. No choice of pin rescues it: $259$ is the best of the four. This is why
the shipped closure stops at $K_{\mathrm{sat}}$, and why imposing
$Q_{\mathrm{sat}}$ waits on an analytic third derivative.
