...
ratios $x_{iY} = \Gamma_{iY}/\Gamma_{iN}$. The vector ratios are SU(6) times a
free factor, one per (meson, multiplet) pair,
%
\begin{equation}
...
\end{equation}
%
with the SU(6) values
$x^{\mathrm{SU(6)}}_{\omega\Lambda} = x^{\mathrm{SU(6)}}_{\omega\Sigma} = 2/3$,
$x^{\mathrm{SU(6)}}_{\omega\Xi} = 1/3$;
$x^{\mathrm{SU(6)}}_{\rho\Lambda} = 0$,
$x^{\mathrm{SU(6)}}_{\rho\Sigma} = 2$,
$x^{\mathrm{SU(6)}}_{\rho\Xi} = 1$; and the hidden-strange vector coupling
normalised to the nucleon $\omega$,
$x^{\mathrm{SU(6)}}_{\phi Y} = g_{\phi Y}/g_{\omega N}
 = \{-\sqrt2/3,\, -\sqrt2/3,\, -2\sqrt2/3\}$ for $\{\Lambda, \Sigma, \Xi\}$,
inheriting the $\omega$ density dependence:
$\Gamma_{\phi Y}(\nB) = x_{\phi Y}\, \Gamma_{\omega N}(\nB)$ (the DD2Y
choice~\cite{Marques2017}).

The nine $y_{M Y}$ are ordinary parameters, each defaulting to $1$, which is
SU(6) exactly and is what DD2Y uses; SU(6) spin-flavour symmetry with ideal
$\omega$--$\phi$ mixing is a quark-model assumption rather than a measurement,
so an inference run must be able to vary them. Nine and not three: SFHoY
breaks $\omega$ and $\phi$ by $1.5$ ($\Lambda, \Sigma$) and $1.875$ ($\Xi$)
while leaving $\rho$ at SU(6)~\cite{Fortin2017}, which neither a per-meson nor
a per-multiplet factor can express. Note that $y_{\phi Y}$ multiplies a
\emph{negative} ratio, so $y_{\phi Y} > 1$ makes $g_{\phi Y}$ more negative;
$y_{\phi\Lambda} = y_{\phi\Sigma} = y_{\phi\Xi} = 0$ is a parametrization with
no hidden-strange sector at all, which is how that sector is switched off,
there being no flag for it.

The scalar ratios are either the published DD2Y values
$R_\sigma = \{0.62, 0.48, 0.32\}$~\cite{Fortin2017} or inverted from
...
regenerates the published $R_\sigma$ column. Equation~\eqref{eq:UY} contains
$\Gamma_{\omega Y} = x_{\omega Y}\Gamma_{\omega N}$, so the scalar coupling
that reproduces a given depth depends on the vector one: the inversion is run
...
The $\Delta$ quartet has no canonical DD2 coupling table and takes no SU(6)
factors: its three ratios $x_{\Delta\sigma}$, $x_{\Delta\omega}$,
$x_{\Delta\rho}$ are free variables directly, defaulting to universal coupling
($x_{\Delta i} = 1$). Alternatively $x_{\Delta\sigma}$ is fixed by the same
inversion from a chosen $U_\Delta \in [-100, -50]$~MeV, at given
$x_{\Delta\omega}$, $x_{\Delta\rho}$.

\paragraph{Three routes to a parameter set.} Model parameters are always
arguments, so all three have to exist. \emph{By name:}
\texttt{Parameters.default()} is the published DD2 table above, and
\texttt{Parameters.named('DD2')} or \texttt{named('DD2Y')} take a published set
by name, an unknown name raising \texttt{KeyError} that lists what there is.
\emph{A new set:} \texttt{dataclasses.replace(Parameters.default(), ...)}, or
\texttt{Parameters.from\_microscopic(...)}, which derives the omitted shape
coefficients $a_i$, $d_i$ from the internal constraints and validates any that
are supplied. Eighteen of the twenty-seven fields carry no default, so bare
...
that go with it. \emph{From nuclear-matter parameters:}
\texttt{nmp.from\_nmp} and \texttt{nmp.invert\_nmp}, imposing
$\{n_\mathrm{sat}, E_\mathrm{sat}, m^*/m, K_\mathrm{sat}, E_\mathrm{sym},
L_\mathrm{sym}\}$.
...
\paragraph{The derivatives, in closed form.} None of these is a finite
difference. Symmetric matter at $T = 0$ carries one self-consistent field, so
writing $S = \Gamma_\sigma(\nB) \sigma$ --- hence $m^* = m_N - S$ --- and
%
\begin{equation}
G(\nB) = \frac{\Gamma_\sigma^2(\nB)}{m_\sigma^2},
\qquad
W(\nB) = \frac{\Gamma_\omega^2(\nB)}{m_\omega^2},
\end{equation}
%
the $\sigma$ gap equation $m_\sigma^2 \sigma = \Gamma_\sigma n_s$ and the
eliminated $\omega_0$ give the closed forms
%
\begin{align}
S &= G(\nB)\, n_s\!\left(m_N - S,\, k_F(\nB)\right),
\label{eq:gap-S} \\
\varepsilon &= \varepsilon_{\mathrm{kin}}(m^*, k_F)
  + \frac{S^2}{2G} + \frac{W \nB^2}{2}, \\
\mu &= E^*_F + W \nB + \frac{W' \nB^2}{2} - \frac{G' n_s^2}{2},
\label{eq:mu-snm}
\end{align}
%
the last two terms of Eq.~\eqref{eq:mu-snm} being
$\Sigma^R = \Gamma_\omega' \omega_0 \nB - \Gamma_\sigma' \sigma n_s$ rewritten
in these variables, and a prime meaning $\mathrm{d}/\mathrm{d}\nB$. Because
$P = \mu \nB - \varepsilon$ at $T = 0$ and $E/A = \varepsilon/\nB - m_N$, one
has $\partial_n(E/A) = P/\nB^2$ and $P' = \nB \mu'$, so at the saturation
point, where $P = 0$,
%
\begin{equation}
K_{\mathrm{sat}} = 9 \nsat\, \mu'(\nsat),
...
\end{equation}
%
The third derivative of $E/A$ is therefore only the second derivative of
$\mu$, and what that needs is $S'$ and $S''$ from Eq.~\eqref{eq:gap-S}
differentiated implicitly. With $n_{s,m} = \partial n_s/\partial m^*$,
$n_{s,k} = \partial n_s/\partial k_F$ and so on, and using
$\mathrm{d}m^*/\mathrm{d}\nB = -S'$,
%
\begin{align}
\frac{\mathrm{d} n_s}{\mathrm{d}\nB}
  &= -n_{s,m} S' + n_{s,k} k_F' , \\
S' \left(1 + G\, n_{s,m}\right)
  &= G' n_s + G\, n_{s,k} k_F' , \\
S'' \left(1 + G\, n_{s,m}\right)
  &= G'' n_s + 2 G' \frac{\mathrm{d} n_s}{\mathrm{d}\nB}
   + G \Big( n_{s,mm} S'^2 - 2 n_{s,mk} S' k_F'
   + n_{s,kk} k_F'^2 + n_{s,k} k_F'' \Big) ,
\end{align}
%
with $k_F = (3\pi^2 \nB/2)^{1/3}$, so $k_F' = k_F/3\nB$ and
$k_F'' = -2k_F/9\nB^2$. The partials of the scalar density
$n_s = (g/4\pi^2)\, m^* [k_F E_F - m^{*2} L]$, $L = \operatorname{arcsinh}
(k_F/m^*)$, $g = 4$, are elementary: those in $k_F$ are the integrand at the
surface, and those in $m^*$ are the moments
%
\begin{equation}
\frac{\partial n_s}{\partial m^*}
  = \frac{g}{2\pi^2} \int_0^{k_F} \frac{k^4}{E_k^3}\, \mathrm{d}k ,
\qquad
\frac{\partial^2 n_s}{\partial m^{*2}}
  = -\frac{3 m^* g}{2\pi^2} \int_0^{k_F} \frac{k^4}{E_k^5}\, \mathrm{d}k ,
\end{equation}
%
which $k = m^* \sinh t$ turns into
$m^{*2}\!\int (\cosh^2 t - 2 + \operatorname{sech}^2 t)\,\mathrm{d}t$ and
$\int \tanh^4 t\, \mathrm{d}t$. $L_{\mathrm{sym}}$ and $K_{\mathrm{sym}}$
follow from Eq.~\eqref{eq:esym} with the same $E^*_F$ derivatives and no
further machinery, since $\Gamma_\rho^2/m_\rho^2$ is a pure exponential.
$Z_{\mathrm{sat}}$, the fourth derivative, is deliberately not reported: it
would require a third derivative of Eq.~\eqref{eq:gap-S}, no closure imposes
it, and the fourth finite difference it would replace spanned
$4.8\times 10^{4}$ on a value of $4547$.

These forms treat the two nucleons as one $g = 4$ gas at the average Dirac
mass, i.e.\ \texttt{nucleon\_mass\_mode="average"}, the convention the
published DD2 nuclear-matter parameters are stated in.
...
c_\omega\}$ against four imposed values, and is closed by pinning two shape
coefficients, $b_\sigma$ and $c_\omega$, at their published values. Two are
needed rather than one because $E_{\mathrm{sat}}$ and $m^*/m$ at fixed
$\nsat$ are blind to the shape coefficients --- they require only $f_i(1) = 1$
--- so only $P$ and $K_{\mathrm{sat}}$ carry shape information and four
coefficients answer to two rows. These two are the best of the six pairs by
condition number ($128$, against $305$ for holding the $\sigma$ shape whole
and $354$ for the $\omega$ shape), since what should remain free is the least
collinear surviving pair. Higher derivatives not imposed ---
$Q_{\mathrm{sat}}$ and $K_{\mathrm{sym}}$ --- are then \emph{predictions} of
the inversion and are reported as such; imposing $Q_{\mathrm{sat}}$ in place
of one pin remains available as an option, and is usable now that the
$Q_{\mathrm{sat}}$ row is analytic (below).

\paragraph{No cross-constraint, and why.} Earlier versions of this inverse
map closed the isoscalar sector with $f''_\sigma(1) = f''_\omega(1)$. That
condition belongs to the DD parametrization, not to DD2. Typel
\cite{Typel2005} imposes $f_\sigma(1) = f_\omega(1) = 1$,
$f''_\sigma(0) = f''_\omega(0) = 0$ and $f''_\sigma(1) = f''_\omega(1)$ on
the rational functions, for the stated purpose of reducing the number of free
parameters, and counts \emph{eight} independent parameters for DD; Typel
\emph{et al.} \cite{Typel2010}, the DD2 paper, states only the first two
conditions and counts \emph{ten}. The difference of one is exactly this
constraint, and the published tables say the same:
$f''_\sigma(1) - f''_\omega(1)$ is $-6.0\times 10^{-8}$ for DD and
$2.200718\times 10^{-3}$ for DD2. It bound the inverse map only ---
\texttt{Parameters.\_\_post\_init\_\_} validates $f_i(1) = 1$ and
$d_i = 1/\sqrt{3 c_i}$ and never checked it, so no forward path saw it --- and
it is now absent from the inverse map as well.

Two numerical caveats are part of the specification. First,
$Q_{\mathrm{sat}}$ and $K_{\mathrm{sym}}$, the two quantities the inversion
predicts rather than imposes, come from the same closed forms as the imposed
rows, so what the closure imposes is exactly what the forward map reports and
a round trip returns its own inputs to $\sim 10^{-11}$. Second, the isoscalar residual surface has spurious basins;
...
Third, the published couplings \emph{are} a root of this closure, which
they were not while the cross-constraint was imposed: all four rows vanish at
the published table, and a round trip through the forward map returns it to
$1.1\times 10^{-5}$. \texttt{InversionStatus} nonetheless reports
\texttt{coupling\_shift}, the maximum relative distance from the seed,
because \emph{converged} and \emph{recovered the published couplings} remain
different statements away from DD2's own point. It also decides the verdict: a
Powell hybrid can give up on its first step and return its starting point bit
for bit, and such a solve is reported as not converged. The residual alone
cannot make that call, since a stall reports whatever residual its seed
carries.

Fourth, $Q_{\mathrm{sat}}$ could not be imposed while it was a third
difference, and now can be. That closure conditions at $259$, and the
$Q_{\mathrm{sat}}$ row used to carry a relative floor near
$1.5\times 10^{-3}$ --- the value spanned $2.48$~MeV over
$h \in [5\times 10^{-5}, 5\times 10^{-4}]$, against $5.2\times 10^{-4}$~MeV
for $K_{\mathrm{sat}}$ --- so a solve inherited $O(0.4)$ of relative coupling
error, and no choice of pin rescued it, $259$ being the best of the four. The
closed forms above remove the floor rather than the amplification: at DD2's
own point that closure went from $\max|\mathrm{residual}| = 1.4\times 10^{-2}$
with $Q_{\mathrm{sat}}$ imposed only to $1.6$~MeV, to $1.5\times 10^{-12}$
with $Q_{\mathrm{sat}}$ imposed to $10^{-10}$~MeV. The default closure
nonetheless still stops at $K_{\mathrm{sat}}$, because it imposes the
nuclear-matter parameters that are quoted and predicts the rest.
