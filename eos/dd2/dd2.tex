...
\midrule
\texttt{cs2\_isothermal} & $c_s^2 = (\partial P/\partial\varepsilon)$ along the
                   mode's own sequence & always \\
...
Y_{L_e}$. It returns the adiabatic sound speed and the adiabatic index
$\Gamma$ at that proton fraction, for nucleonic matter.
...
\paragraph{Isothermal against adiabatic.} A returned name says which thermal
variable is held, which is why no quantity in this package is called a bare
\texttt{cs2}. Both freezes here hold $T$, so both give an isothermal speed;
what separates them is the composition axis, carried by the \texttt{frozen=}
argument rather than by the key. \texttt{frozen='equilibrium'} differentiates
along the mode's own sequence, \texttt{frozen='composition'} at frozen
particle fractions. The adiabatic speed, larger by $C_P/C_V$ at $T > 0$, is
not computed by this model; \texttt{docs/DEFERRED.md} records the gap.
