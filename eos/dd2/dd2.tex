...
\midrule
\texttt{cs2\_eq} & $c_s^2 = (\partial P/\partial\varepsilon)$ along the
...
variable is held: \texttt{cs2\_eq} is the equilibrium sound speed along the
sequence, and the \texttt{composition} freeze's is the adiabatic one. The two
differ at $T > 0$ by the factor $C_P/C_V$, which is why no quantity in this
package is called a bare \texttt{cs2}.
