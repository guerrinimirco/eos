...
the nucleon ones. The vector ratios are SU(6) times a free factor, one per
(meson, multiplet) pair,
%
\begin{equation}
x_{M Y} = y_{M Y}\, x_{M Y}^{\mathrm{SU(6)}},
\qquad M \in \{\omega, \rho, \phi\},\quad
Y \in \{\Lambda, \Sigma, \Xi\},
\label{eq:su6break}
\end{equation}
%
with $x^{\mathrm{SU(6)}}_{\omega Y} = g_{\omega Y}/g_{\omega N}
 = \{2/3,\, 2/3,\, 1/3\}$,
$x^{\mathrm{SU(6)}}_{\rho Y} = g_{\rho Y}/g_{\rho N} = \{0,\, 2,\, 1\}$ and
$x^{\mathrm{SU(6)}}_{\phi Y} = g_{\phi Y}/g_{\omega N}
 = \{-\sqrt2/3,\, -\sqrt2/3,\, -2\sqrt2/3\}$ for $\{\Lambda, \Sigma, \Xi\}$.

The nine $y_{M Y}$ are ordinary parameters, each defaulting to $1$, which is
SU(6) exactly and is SFHoY$^*$; SU(6) spin-flavour symmetry with ideal
$\omega$--$\phi$ mixing is a quark-model assumption rather than a measurement,
so an inference run must be able to vary them. SFHoY is
$y_{\omega\Lambda} = y_{\phi\Lambda} = y_{\omega\Sigma} = y_{\phi\Sigma}
 = 1.5$ and $y_{\omega\Xi} = y_{\phi\Xi} = 1.875$ with every $y_{\rho Y} = 1$:
Ref.~\cite{Fortin2017} \S2.2 rescales the $\omega$ and $\phi$ hyperon
couplings as $g_{M\Lambda} = 1.5\, g_{M\Lambda}^{\mathrm{SU(6)}}$,
$g_{M\Sigma} = 1.5\, g_{M\Sigma}^{\mathrm{SU(6)}}$,
$g_{M\Xi} = 1.875\, g_{M\Xi}^{\mathrm{SU(6)}}$, and its Table~1 lists the
result ($R_{\omega\Lambda} = 1$, $R_{\omega\Xi} = 0.62$,
$R_{\phi\Xi} = -1.77$). That set is why nine factors are needed and not three:
$\rho$ is untouched while $\omega$ and $\phi$ are not, which neither a
per-meson nor a per-multiplet factor can express. Note that $y_{\phi Y}$
multiplies a \emph{negative} ratio, so $y_{\phi Y} > 1$ makes $g_{\phi Y}$
more negative; $y_{\phi\Lambda} = y_{\phi\Sigma} = y_{\phi\Xi} = 0$ is a
parametrization with no hidden-strange sector at all, which is
\texttt{SFHo\_2fam} and is how that sector is switched off, there being no
flag for it.

The scalar ratios are either the published SFHoY values or inverted from the
hyperon single-particle potentials in symmetric matter at saturation,
%
\begin{equation}
...
Equation~\eqref{eq:UY} contains $g_{\omega Y}$, so the scalar coupling that
reproduces a given depth depends on the vector one: the inversion is run
\emph{after} Eq.~\eqref{eq:su6break}, and rescaling a $y_{\omega Y}$ on a set
whose scalar couplings are already fixed moves $U_Y$ instead. Which of the two
is held --- the depths or the ratios --- is the caller's physics and is an
explicit argument, never a default; it is also what an inversion of the
nucleon sector has to be told, since the hyperon couplings are stored
absolutely and are defined against nucleon couplings such an inversion moves.

The $\Delta$ quartet has no canonical SFHo coupling table and takes no SU(6)
factors: its three ratios $x_{\Delta\sigma}$, $x_{\Delta\omega}$,
$x_{\Delta\rho}$ are free variables directly, defaulting to $1.15$, $1$ and
$1$.
...
$T > 0$ the solver adds a $\mu = 0$ massless neutrino gas over the flavours
the composition does \emph{not} track, multiplying the single-flavour result
(antiparticles included) into the totals:
%
\begin{equation}
P \mathrel{+}= N_\nu P_\nu(0, T) , \qquad
\varepsilon \mathrel{+}= N_\nu \varepsilon_\nu(0, T) , \qquad
s \mathrel{+}= N_\nu s_\nu(0, T) ,
\label{eq:thermalnu}
\end{equation}
%
with $N_\nu = 3$ where no neutrino flavour is tracked and $N_\nu = 2$ in a
mode that holds $Y_{L_e}$. It carries no conserved charge and no lepton
number, so it touches nothing else. The factor is exactly $3$ in the
free-streaming modes, measured: at $\nB = 0.3\ \mathrm{fm}^{-3}$,
...
The count follows the mode, and \texttt{beta\_eq\_neutrino\_trapped} does not
refuse the combination: that mode carries the electron neutrino explicitly at
its own $\munue$, so a three-flavour $\mu = 0$ gas on top would count $\nu_e$
twice and the gas is two flavours there. The flag is defined by what it does
\emph{not} cover, so under trapping --- where the $e$ family is tracked --- it
means the $\mu$ and $\tau$ families, which are free-streaming and carry no
lepton number the mode constrains. Nothing here is deferred.
...
\subsection{The density derivatives in closed form}
\label{sec:snmderiv}

Every derivative above is taken analytically rather than by finite
difference. Everything in this subsection is at $T = 0$ and in natural units,
with $n$ the baryon density in MeV$^3$ and a prime meaning $d/dn$.

Symmetric matter at $Y_C = 1/2$ carries $n_p = n_n = n/2$ exactly, so
$\rho = \phi = 0$ identically and the two nucleons are two $g = 2$ gases
sharing one Fermi momentum $k_F = (3\pi^2 n/2)^{1/3}$ but not one Dirac mass:
$m^*_p = m_p - g_{\sigma N}\sigma$ and $m^*_n = m_n - g_{\sigma N}\sigma$
differ by $m_n - m_p$. Abbreviating the two field polynomials of
Sec.~\ref{sec:fields} at $\rho = 0$,
%
\begin{equation}
\Phi(\sigma) = m_\sigma^2 \sigma + g_2 \sigma^2 + g_3 \sigma^3 ,
\qquad
\Psi(\omega) = m_\omega^2 \omega + c_3 \omega^3 ,
\end{equation}
%
the field equations are
%
\begin{equation}
\Phi(\sigma) = g_{\sigma N}\, n_s(\sigma, n),
\qquad n_s = n_s^p + n_s^n ,
\qquad\qquad
\Psi(\omega) = g_{\omega N}\, n ,
\label{eq:snmfields}
\end{equation}
%
and the $\omega$ equation contains no scalar density at all, so $\omega$ is a
function of $n$ alone, exactly and independently of the scalar sector:
%
\begin{equation}
\omega' = \frac{g_{\omega N}}{\Psi'(\omega)} ,
\qquad
\omega'' = -\,\frac{\Psi''(\omega)\, \omega'^2}{\Psi'(\omega)} ,
\qquad
\Psi'(\omega) = m_\omega^2 + 3 c_3 \omega^2 ,
\quad
\Psi''(\omega) = 6 c_3 \omega .
\end{equation}
%
The energy density and the mean chemical potential are
%
\begin{equation}
\varepsilon = \varepsilon_{\mathrm{kin}}(m^*_p, k_F)
            + \varepsilon_{\mathrm{kin}}(m^*_n, k_F)
            + V(\sigma) + \tfrac12 m_\omega^2 \omega^2
            + \tfrac34 c_3 \omega^4 ,
\qquad
\bar\mu = \tfrac12 \big( E^{*}_{F,p} + E^{*}_{F,n} \big)
        + g_{\omega N} \omega ,
\end{equation}
%
with $V(\sigma) = \tfrac12 m_\sigma^2\sigma^2 + \tfrac13 g_2 \sigma^3
+ \tfrac14 g_3 \sigma^4$ and $E^*_{F,i} = \sqrt{k_F^2 + m^{*2}_i}$. The mean
$\bar\mu$ of $\mu_p$ and $\mu_n$ is the right potential here because the sweep
holds $Y_C = 1/2$: then $d\varepsilon = \mu_p\, dn_p + \mu_n\, dn_n = \bar\mu\, dn$, so $\varepsilon' = \bar\mu$ even though
$\mu_p \neq \mu_n$. With $P = \bar\mu n - \varepsilon$ and
$E/A = \varepsilon/n - m_N$ this gives
%
\begin{equation}
(E/A)' = \frac{P}{n^2} ,
\qquad
P' = n \bar\mu' ,
\qquad
K_{\mathrm{sat}} = 9 n \bar\mu' ,
\qquad
Q_{\mathrm{sat}} = 27 n \big( n \bar\mu'' - 3 \bar\mu' \big) ,
\label{eq:KQfrommu}
\end{equation}
%
the last two at $P = 0$, i.e.\ at $\nsat$. A \emph{third} derivative of $E/A$
is therefore only a \emph{second} derivative of $\bar\mu$, and the gap
equation needs differentiating twice rather than three times.

What that costs is $\sigma'$ and $\sigma''$ from
Eq.~\eqref{eq:snmfields} differentiated implicitly. Writing
$n_{s,m} = \partial n_s/\partial m^*$, $n_{s,k} = \partial n_s/\partial k_F$
and so on --- each summed over the two gases --- and using
$dm^*_i/dn = -g_{\sigma N}\sigma'$ and
$k_F' = k_F/3n$, $k_F'' = -2k_F/9n^2$,
%
\begin{align}
D &\equiv \Phi'(\sigma) + g_{\sigma N}^2\, n_{s,m} , \\
D\, \sigma' &= g_{\sigma N}\, n_{s,k}\, k_F' , \\
D\, \sigma'' &= -\Phi''(\sigma)\, \sigma'^2
  + g_{\sigma N}\Big( g_{\sigma N}^2 n_{s,mm}\, \sigma'^2
  - 2 g_{\sigma N} n_{s,mk}\, \sigma' k_F'
  + n_{s,kk}\, k_F'^2 + n_{s,k}\, k_F'' \Big) ,
\end{align}
%
with $\Phi'(\sigma) = m_\sigma^2 + 2 g_2\sigma + 3 g_3\sigma^2$ and
$\Phi''(\sigma) = 2 g_2 + 6 g_3 \sigma$. The scalar-density partials of one
$g = 2$ gas are elementary. With $E_k = \sqrt{k^2 + m^{*2}}$,
$E_F = \sqrt{k_F^2 + m^{*2}}$ and $\Lambda = \operatorname{arcsinh}(k_F/m^*)$,
%
\begin{equation}
n_s = \frac{g}{4\pi^2} m^* \big( k_F E_F - m^{*2}\Lambda \big) ,
\qquad
\frac{\partial n_s}{\partial m^*} = \frac{g}{2\pi^2}\!\int_0^{k_F}\!\!
   \frac{k^4}{E_k^3}\, dk ,
\qquad
\frac{\partial^2 n_s}{\partial m^{*2}} = -\frac{3 g\, m^*}{2\pi^2}\!
   \int_0^{k_F}\!\! \frac{k^4}{E_k^5}\, dk ,
\end{equation}
%
and the substitution $k = m^*\sinh t$ turns the two integrals into
$m^{*2}\!\int(\cosh^2 t - 2 + \operatorname{sech}^2 t)\, dt$ and
$\int \tanh^4 t\, dt$, so that
%
\begin{align}
\frac{\partial n_s}{\partial m^*} &= \frac{g}{2\pi^2}
   \left( \tfrac12 k_F E_F - \tfrac32 m^{*2}\Lambda
          + \frac{m^{*2} k_F}{E_F} \right) ,
&
\frac{\partial^2 n_s}{\partial m^{*2}} &= -\frac{3 g\, m^*}{2\pi^2}
   \left( \Lambda - \frac{k_F}{E_F}
          - \frac13 \frac{k_F^3}{E_F^3} \right) , \\
\frac{\partial n_s}{\partial k_F} &= \frac{g}{2\pi^2}\frac{k_F^2 m^*}{E_F} ,
&
\frac{\partial^2 n_s}{\partial m^* \partial k_F} &= \frac{g}{2\pi^2}
   \frac{k_F^4}{E_F^3} ,
\end{align}
%
and $\partial^2 n_s/\partial k_F^2 = (g/2\pi^2)\, m^* k_F (2E_F^2 - k_F^2)
/ E_F^3$. The Fermi energies then follow with
$dm^*_i/dn = -g_{\sigma N}\sigma'$ and
$d^2 m^*_i/dn^2 = -g_{\sigma N}\sigma''$,
%
\begin{equation}
E^{*\prime}_{F,i} = \frac{k_F k_F' - m^*_i g_{\sigma N}\sigma'}{E^*_{F,i}} ,
\qquad
E^{*\prime\prime}_{F,i} =
  \frac{k_F'^2 + k_F k_F'' + (g_{\sigma N}\sigma')^2
        - m^*_i g_{\sigma N}\sigma''}{E^*_{F,i}}
  - \frac{\big(E^{*\prime}_{F,i}\big)^2}{E^*_{F,i}} ,
\end{equation}
%
which close $\bar\mu' = \tfrac12(E^{*\prime}_{F,p} + E^{*\prime}_{F,n})
+ g_{\omega N}\omega'$ and
$\bar\mu'' = \tfrac12(E^{*\prime\prime}_{F,p} + E^{*\prime\prime}_{F,n})
+ g_{\omega N}\omega''$, and with them Eq.~\eqref{eq:KQfrommu}.

The isovector pair costs nothing further. Writing $u = k_F^2$ (so
$u' = 2u/3n$, $u'' = -2u/9n^2$) and $R = g_{\rho N}^2/Q$ with
$Q = m_\rho^2 + 2A$, Eq.~\eqref{eq:esym} differentiates to
%
\begin{align}
E_{\mathrm{sym}}' &= \frac{u'}{6 E^*_{F,n}}
   - \frac{u\, E^{*\prime}_{F,n}}{6 E^{*2}_{F,n}}
   + \frac{R + n R'}{8} , \\
E_{\mathrm{sym}}'' &= \frac16 \left(
     \frac{u''}{E^*_{F,n}}
   - \frac{2 u' E^{*\prime}_{F,n}}{E^{*2}_{F,n}}
   - \frac{u\, E^{*\prime\prime}_{F,n}}{E^{*2}_{F,n}}
   + \frac{2 u\, \big(E^{*\prime}_{F,n}\big)^2}{E^{*3}_{F,n}} \right)
   + \frac{2 R' + n R''}{8} ,
\end{align}
%
with
$R' = -2 g_{\rho N}^2 A' / Q^2$ and
$R'' = -2 g_{\rho N}^2 A''/Q^2 + 8 g_{\rho N}^2 A'^2 / Q^3$, and
%
\begin{equation}
A' = A_\sigma \sigma' + A_\omega \omega' ,
\qquad
A'' = A_{\sigma\sigma}\sigma'^2 + A_{\omega\omega}\omega'^2
    + A_\sigma \sigma'' + A_\omega \omega'' .
\end{equation}
%
There is no $A_{\sigma\omega}$: $f$ of Eq.~\eqref{eq:Afunction} is a sum of a
$\sigma$ series and an $\omega$ series and is therefore separable, which is a
property of the SFHo form rather than a truncation. $E^*_{F,n}$ carries the
neutron Dirac mass here because Eq.~\eqref{eq:esym} does.

$Z_{\mathrm{sat}}$, the fourth derivative of $E/A$, is deliberately not
reported: it would need a third derivative of the gap equation, no closure
imposes it, and nobody quotes it.

Why analytic and not a stencil: the finite differences these expressions
replaced were taken on a quantity that is itself the output of a nonlinear
solve, so their accuracy was bounded by the solver rather than by $h$. The
verification suite ties the two together by requiring each analytic value to
be the $h \to 0$ limit the stencil converges to, within the stencil's own
scatter.
...
\texttt{Parameters.default()} is the nucleonic SFHo table above, and
\texttt{Parameters.named(name)} takes any of the five published sets ---
\texttt{SFHo\_Nucleonic}, \texttt{SFHoY\_Fortin}, \texttt{SFHoY*\_Fortin},
\texttt{SFHo\_2fam}, \texttt{SFHo\_2fam\_phi} --- an unknown name raising
\texttt{KeyError} that lists them. \emph{A new set:} every field carries a
default, so \texttt{Parameters(g\_rho\_N=...)} builds one field by field and
\texttt{dataclasses.replace} modifies one already in hand. \emph{From
nuclear-matter parameters:} \texttt{nmp.from\_nmp} and
\texttt{nmp.invert\_nmp}, imposing $\{n_\mathrm{sat}, E_\mathrm{sat}, m^*/m,
K_\mathrm{sat}, E_\mathrm{sym}, L_\mathrm{sym}\}$.
...
carries is a declared property of the phase, never an engine assumption. All ten
models spell this surface \texttt{thermo\_from\_mu}; SFHo and DID are the two
that also carry a lower \texttt{thermo\_from\_fields} beneath it.
%
\begin{quote}
