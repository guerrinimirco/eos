...
The inverse map is not yet written. Its isoscalar half is the classical
Boguta--Bodmer inversion, $\{\nsat, E_{\mathrm{sat}}, K_{\mathrm{sat}},
m^*/m\}$ against $\{g_{\sigma N}, g_{\omega N}, g_2, g_3\}$ at fixed
$m_\sigma$, $m_\omega$, $c_3$. Its isovector half is not well posed as stated:
two conditions $\{E_{\mathrm{sym}}, L_{\mathrm{sym}}\}$ face $g_{\rho N}$ plus
the nine shape coefficients of Eq.~\eqref{eq:Afunction}, so a closure --- which
two of the ten are freed --- has to be chosen, and that choice is physics
rather than bookkeeping, because it decides how $E_{\mathrm{sym}}$ behaves
above saturation where no nuclear-matter parameter constrains it.
