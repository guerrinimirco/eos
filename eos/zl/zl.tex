...
published set of Constantinou et al., and
\texttt{Parameters.named('ZL\_Constantinou')} takes it by name. ZL ships
exactly one set, so the map has a single entry; it exists so that a caller
sweeping parameter sets need not know which models happen to have more than
one. \emph{A new set:} every field carries a default, so
\texttt{Parameters(a0=..., gamma=...)} names only what changes, and
\texttt{dataclasses.replace} modifies one already in hand. \emph{From
nuclear-matter parameters:} \texttt{nmp.from\_nmp} and
\texttt{nmp.invert\_nmp}, in \textbf{closed form} --- the interaction enters
the nuclear-matter parameters linearly in $a_0, b_0, a_1, b_1$ once the two
exponents are known, and the exponents follow from ratios of the isoscalar
data, so there is no seed, no basin and no restart count. Six couplings against
the five nuclear-matter parameters of the standard list still leaves one free
choice, and the caller names it: either $\gamma_1$, or a sixth datum
$K_{\mathrm{sym}}$ (Sec.~\ref{sec:nmp}). \texttt{nmp.compute\_nmp} is the
forward direction.
...
r_5 &= \nC - n_e(\mu_e, T) \;\;(= n_p - n_e) . &&
\end{aligned}
\label{eq:residual_beta}
\end{equation}
%
Row $r_4$ is beta equilibrium with free-streaming neutrinos ($\munue = 0$),
$r_5$ total electric neutrality. \textbf{The neutrality row is written
$\nC - n_e$ in the two $\beta$-equilibrium modes and $n_e - \nC$ in
\texttt{fixed\_YC}}, which is the same condition with the sign reversed. The
root is of course unchanged; the residual is not, and this document states each
mode's row as the code assembles it rather than picking one spelling and
...
$r_1$, $r_2$ and $r_5' = n_e(\mu_e, T) - \nC$ --- the neutrality condition
with the sign it carries in this mode; with \texttt{leptons=False},
$x = [\mu_p, \mu_n]$ and the rows are $r_1, r_2$ alone, the result being
electrically charged nucleonic matter --- what a mixed-phase construction
needs per pure phase before global neutrality is imposed. All rows of this
mode balance a density, so all of their scales are $\nB$.
...
The forward map is \texttt{nmp.compute\_nmp}; the inverse,
\texttt{nmp.invert\_nmp}, is \textbf{closed form}. ZL carries six couplings
--- $a_0, b_0, \gamma, a_1, b_1, \gamma_1$ --- against the five nuclear-matter
parameters of the standard list, so imposing that list leaves one free choice.
The caller names it and the rest is algebra rather than a root find, which is
what distinguishes this inversion from every other one in the package: no seed,
no basin, no restart count, and therefore no inversion lottery in an inference
run over ZL couplings.

Write $E_{\mathrm{b}} = E_{\mathrm{sat}} - E_{\mathrm{sat}}^{\mathrm{K}}$,
$K_{\mathrm{b}} = K_{\mathrm{sat}} - K_{\mathrm{sat}}^{\mathrm{K}}$,
$S_{\mathrm{b}} = E_{\mathrm{sym}} - E_{\mathrm{sym}}^{\mathrm{K}}$ and
$L_{\mathrm{b}} = L_{\mathrm{sym}} - L_{\mathrm{sym}}^{\mathrm{K}}$ for the
interaction remainders, each the total minus the free-Fermi-gas value at
$n_0$, and let $P^{\mathrm{K}}_0$ be the kinetic pressure there. Setting the
functional's $n_0$ equal to the requested $\nsat$ --- so that saturation is
\emph{imposed} at $u = 1$ rather than found --- the conditions
%
\begin{equation}
a_0 + b_0 = E_{\mathrm{b}}, \qquad
n_0 (a_0 + \gamma b_0) = -P^{\mathrm{K}}_0, \qquad
9 \gamma (\gamma - 1) b_0 = K_{\mathrm{b}}
\label{eq:isoscalar_conditions}
\end{equation}
%
(the second being $P(\nsat) = 0$) invert to, with
$X = E_{\mathrm{b}} n_0 + P^{\mathrm{K}}_0$ and
$D = (9 E_{\mathrm{b}} + K_{\mathrm{b}}) n_0^2 + 9 P^{\mathrm{K}}_0 n_0$,
%
\begin{equation}
\gamma = -\frac{K_{\mathrm{b}} n_0}{9 X}, \qquad
b_0 = \frac{9 X^2}{D}, \qquad
a_0 = \frac{K_{\mathrm{b}} E_{\mathrm{b}} n_0^2
      - 9 P^{\mathrm{K}}_0 (E_{\mathrm{b}} n_0 + P^{\mathrm{K}}_0)}{D} ,
\label{eq:invert_isoscalar}
\end{equation}
%
and the two $\delta^2$ conditions then give the isovector pair
%
\begin{equation}
b_1 = \frac{3 S_{\mathrm{b}} - L_{\mathrm{b}} + 3 b_0 (1 - \gamma)}
           {3 (1 - \gamma_1)}, \qquad
a_1 = \frac{3 \gamma_1 S_{\mathrm{b}} - L_{\mathrm{b}}
       + 3 a_0 (\gamma_1 - 1) + 3 b_0 (\gamma_1 - \gamma)}{3 (\gamma_1 - 1)} .
\label{eq:invert_isovector}
\end{equation}
%
For $\{\nsat = 0.16~\mathrm{fm}^{-3}, E_{\mathrm{sat}} = -16,
K_{\mathrm{sat}} = 250, E_{\mathrm{sym}} = 31.6, L_{\mathrm{sym}} = 43\}$~MeV
with $\gamma_1 = 2.45$ these return $a_0 = -96.6555$, $b_0 = 58.8619$,
$\gamma = 1.39854$, $a_1 = -25.1985$, $b_1 = 7.1850$, and the round trip
through \texttt{compute\_nmp} recovers $\nsat$ to $10^{-14}$,
$E_{\mathrm{sat}}$ to $6 \times 10^{-13}$ and $E_{\mathrm{sym}}$ to
$5 \times 10^{-7}$, with $K_{\mathrm{sat}}$ and $L_{\mathrm{sym}}$ to $10^{-2}$
--- the forward map's own finite-difference stencil, not the algebra.

\paragraph{Two conventions.} Both give silently wrong answers if mixed, and
neither is visible in the equations above. First, the \emph{rest mass}: each
remainder is the interaction piece only because both sides of the subtraction
are binding energies, \texttt{energy\_per\_baryon} having removed
$m_p n_p + m_n n_n$ from $\epsilon$; a derivation carrying an explicit
$+\,m_H$ reads the same symbol the other way. Second, \emph{which}
$E_{\mathrm{sym}}$: this package uses the $\delta^2$ curvature, while
Constantinou et al.\ quote the full pure-neutron minus symmetric step. On the
shipped set those read $30.848 / 41.270$ and $31.561 / 42.718$~MeV --- one
functional, two conventions. The familiar target $\{31.6, 43\}$ is thus the
shipped set stated in the \emph{second} convention, which is precisely why
putting it through Eq.~\eqref{eq:invert_isovector} returns $a_1 = -25.20$,
$b_1 = 7.19$ where the shipped set carries $-26.06$ and $7.34$. Neither is
wrong; they answer different questions.

\paragraph{$Q_{\mathrm{sat}}$ is not free in ZL.} The interaction carries a
single power-law term, so its skewness and its incompressibility come from the
same $b_0 u^{\gamma}$:
%
\begin{equation}
Q_{\mathrm{sat}}^{\mathrm{pot}}
  = 27\, \gamma (\gamma - 1)(\gamma - 2)\, b_0
  = 3 (\gamma - 2)\, K_{\mathrm{sat}}^{\mathrm{pot}}
\label{eq:qsat_rigidity}
\end{equation}
%
exactly. Once $\{\nsat, E_{\mathrm{sat}}, K_{\mathrm{sat}}\}$ fix $\gamma$ and
$b_0$ through Eq.~\eqref{eq:invert_isoscalar}, $Q_{\mathrm{sat}}$ follows: it
cannot serve as the sixth datum, and a prior over
$(K_{\mathrm{sat}}, Q_{\mathrm{sat}})$ in ZL lives on a curve rather than in a
plane. Nothing isovector reaches $\gamma$ or $b_0$, which is how the identity
is pinned in the test suite --- moving $E_{\mathrm{sym}}$, $L_{\mathrm{sym}}$
and $\gamma_1$ leaves the predicted $Q_{\mathrm{sat}}$ bit-identical, an exact
statement where comparing the third-derivative stencil against
Eq.~\eqref{eq:qsat_rigidity} could only be checked to its own truncation error.

\paragraph{$K_{\mathrm{sym}}$ may be imposed, and only here.} The isovector
sector has \emph{three} knobs, $a_1, b_1, \gamma_1$, against three isovector
data, so $\gamma_1$ can be determined rather than chosen. From
%
\begin{equation}
K_{\mathrm{sym}}^{\mathrm{pot}}
  = 9 \left[ \gamma_1 (\gamma_1 - 1) b_1 - \gamma (\gamma - 1) b_0 \right] ,
\label{eq:ksym_pot}
\end{equation}
%
and noting that $E_{\mathrm{sym}}$ and $L_{\mathrm{sym}}$ alone already fix the
product $(\gamma_1 - 1) b_1 = L_{\mathrm{b}}/3 - S_{\mathrm{b}} - X/n_0$, the
exponent follows in closed form as well,
%
\begin{equation}
\gamma_1 = \frac{K_{\mathrm{sym}}^{\mathrm{pot}}/9
                 + \gamma (\gamma - 1) b_0}{(\gamma_1 - 1) b_1} ,
\label{eq:invert_gamma1}
\end{equation}
%
after which Eq.~\eqref{eq:invert_isovector} supplies $a_1$ and $b_1$ as before.
ZL is the only model in this package that can impose $K_{\mathrm{sym}}$; every
other hadronic model has two isovector knobs and reports it as a prediction.
It is offered rather than imposed: $K_{\mathrm{sym}}$ is passed in the
nuclear-matter-parameter dictionary \emph{instead of} $\gamma_1$, and passing
both, or neither, is refused --- they are two names for the same freedom.
...
\texttt{error}     & --- & largest scaled residual, dimensionless \\
\midrule
\texttt{n\_B}, \texttt{T} & $\nB$, $T$ & $\mathrm{fm}^{-3}$, MeV: the
                                          conditions \\
\texttt{Y\_C}, \texttt{Y\_S}, \texttt{Y\_L}
   & $Y_C, Y_S, Y_{L_e}$ & the fractions; $Y_S \equiv 0$ always \\
\midrule
\texttt{mu\_p}, \texttt{mu\_n} & $\mu_p, \mu_n$ & MeV, \emph{physical}
                                                  potentials \\
\texttt{mu\_e}, \texttt{mu\_nu} & $\mu_e, \munue$ & MeV \\
\texttt{mu\_B}, \texttt{mu\_C}, \texttt{mu\_S}, \texttt{mu\_L}
   & $\muB, \muC, \muS, \mu_{L}$ & MeV; $\muS = 0$ by convention \\
\midrule
\texttt{n\_p}, \texttt{n\_n}, \texttt{n\_e}, \texttt{n\_nu}
   & $n_p, n_n, n_e, n_{\nu_e}$ & $\mathrm{fm}^{-3}$ \\
\midrule
\texttt{P\_total}, \texttt{e\_total}, \texttt{s\_total}
   & $P$, $\varepsilon$, $s$ & $\mathrm{MeV\,fm^{-3}}$, $\mathrm{MeV\,fm^{-3}}$,
                               $\mathrm{fm}^{-3}$ \\
\midrule
\texttt{Y\_p}, \texttt{Y\_n}, \texttt{Y\_e} & $n_i/\nB$ & per-species
                                                          fractions \\
\bottomrule
\end{tabular}
\end{center}

When \texttt{converged} is \texttt{False} every other field holds the best
iterate reached, which is not a physical state.

Two of these deserve a sentence of their own.

\textbf{$s$} is the sum of the sectors' entropy densities, each obtained
through Eq.~\eqref{eq:si}, the free-gas Euler identity at that sector's
\emph{effective} potential. The interaction never enters $s$, because $V$
depends on the densities and not on $T$. The model therefore does not compute
$s$ from the total Euler relation, which is what leaves
Eq.~\eqref{eq:euler} available as an independent check.

\textbf{There is no scalar density $n_s$}, and its absence is physics rather
than an omission. A scalar density is what a scalar field couples to; ZL has
no scalar field and no Dirac effective mass, so the identity
$n_s = (\varepsilon - 3P)/m^*$ has no $m^*$ to divide by and the quantity is
not defined. This is the one returned-quantity requirement of the repository
conventions that this model discharges by not having the quantity.

\subsection{A table row}

\texttt{eos\_table} flattens each solved point into the long-format row
\texttt{eos.general.table\_io} writes, keyed so that a nucleonic table and a
hybrid table concatenate without renaming.

\section{The API surface}
\label{sec:api}

Three entry points, the same three every model exposes, with the parameters
always the first argument and the mode always required:
%
\begin{itemize}
\item \texttt{eos\_point(par, mode, species, n\_B=, T= \textbar\ SnB=,
      leptons=, **conditions)} --- one state, returned inside a
      \texttt{PointResult} carrying \texttt{ok}, a \texttt{message} and the
      \texttt{point}. Exactly one of \texttt{T} and \texttt{SnB}.
\item \texttt{eos\_table(par, mode, species, axes, fixed=, leptons=,
      progress=, verbose=)} --- a solved grid over
      $\{\nB\} \times \{T\} \times$ the fraction axes the mode fixes, the
      density axis warm-started. \texttt{progress} is called once per
      completed line with $\{$\texttt{mode}, \texttt{line}, \texttt{n\_lines},
      \texttt{temp\_key}, \texttt{temp}, \texttt{fracs}, \texttt{n\_solved},
      \texttt{n\_requested}, \texttt{elapsed\_s}$\}$, the same dictionary in
      every model. Deep solver code never prints.
\item \texttt{eos\_response(par, mode, species, frozen='equilibrium', n\_B=,
      T=, leptons=, rel\_step=, **conditions)} --- second derivatives.
\end{itemize}

\texttt{eos\_response} implements \texttt{frozen='equilibrium'} and nothing
else. Both quantities are central differences over a relative step
\texttt{rel\_step} (default $10^{-3}$) in the variable differentiated:
%
\begin{equation}
c_{s,T}^2 = \left.\frac{\partial P}{\partial\varepsilon}\right|_{T}
\quad\text{under \texttt{cs2\_isothermal}} ,
\qquad
C_V = \frac{T}{\nB}\left.\frac{\partial s}{\partial T}\right|_{\nB}
\quad\text{under \texttt{C\_V}, at $T > 0$ only.}
\end{equation}
%
Every other freeze of the specification --- frozen composition, frozen
conserved fractions, the leptonic re-neutralization variants --- and the
susceptibility matrix $\chi_{ab}$ raise \texttt{NotImplementedError} naming
the gap.
...
%
\begin{enumerate}
\item \textbf{Euler relation}, Eq.~\eqref{eq:euler}, at $10^{-8}$ relative.
\item \textbf{Free energy}, $f = \varepsilon - Ts = -P + \sum_i \mu_i n_i$, at
      $10^{-8}$.
\item \textbf{Interaction identities}: the two expressions for
      $P_{\mathrm{int}}$ in Eq.~\eqref{eq:Pint} agree, and $\muHv{i}$ is the
      numerical derivative of $V$.
\item \textbf{Mode closures}: each mode's own conditions at its own solution,
      at $10^{-8}$.
\item \textbf{Free-gas limit}: with the interaction switched off the solved
      state is two free Fermi gases at the physical potentials, at $10^{-10}$.
\item \textbf{Isospin symmetry}: at $Y_C = 0.5$ symmetric matter comes back
      with $\mu_p = \mu_n$, i.e.\ $\muC = 0$, and $n_p = n_n$, at $10^{-10}$.
      Both nucleons carry the same mass and Eq.~\eqref{eq:V} is symmetric
      under $n_p \leftrightarrow n_n$, so this must hold exactly; it catches a
      sign slip in the isovector term, which is otherwise invisible in the
      totals.
\item \textbf{Residual gate}: every state the suite solved is inside
      $10^{-10}$.
\item \textbf{Causality}: $0 \le c_s^2 \le 1$ along the cold
      $\beta$-equilibrium sequence.
\item \textbf{No strangeness}: $\nS = 0$ and $\muS = 0$ in every returned
      state, and \texttt{fixed\_YC\_YS} raises.
\item \textbf{Nuclear-matter parameters}: the five published values of
      Sec.~\ref{sec:nmp} reproduce at the precision they are published to.
\end{enumerate}
