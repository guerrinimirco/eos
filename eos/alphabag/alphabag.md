...
MIT configuration rather than a broken one. Both values are legal in the
unpaired modes; in `cfl` the flag raises, for the reason given in that
section.
...
The gluon term is not part of (Pcfl), and it is not added to the totals
either: in the `cfl` mode `gluons=True` RAISES. Colour-flavour locking breaks
`SU(3)_c x U(1)_Q` to a single unbroken `U(1)_Qtilde`, generated by a mixture
of the electric charge and the eighth colour generator, so of the nine gauge
bosons exactly one stays massless -- the rotated photon, to which the medium
is transparent -- while all eight gluons acquire Meissner masses. The free gas
(gluons) writes down is `g_g = 2 x 8 = 16` massless degrees of freedom, which
is the wrong count and the wrong dispersion for this phase; it is refused here
rather than corrected, because a Meissner mass is not the fix. The light
bosonic sector the locked phase actually has is the superfluid phonon and the
pseudo-Goldstone meson octet, a different sector, and it is not implemented
(see `docs/DEFERRED.md`).

The photon sector is unaffected and stays available: the rotated photon is a
free massless gas of two polarizations, so at leading order it contributes the
same `photon_thermo(T)` the unpaired phase carries. The thermal neutrino gas
raises here too, for a different and non-physical reason -- the paired phase
has never carried it, and the asymmetry is preserved deliberately because
closing it moves published CFL tables.
...
Accordingly ``eos.alphabag`` **raises** on `fixed_YC_YS` with the flag on.
...
The unknown vector loses the strange entries with the flavour:

| mode | three-flavour `x` | two-flavour `x` |
|---|---|---|
| `beta_eq_neutrinoless` | `(mu_u, mu_d, mu_s, mu_e)` | `(mu_u, mu_d, mu_e)` |
| `beta_eq_neutrino_trapped` | `(mu_u, mu_d, mu_s, mu_e, mu_nue)` | `(mu_u, mu_d, mu_e, mu_nue)` |
| `fixed_YC` | `(mu_u, mu_d, mu_s)` | `(mu_u, mu_d)` |

and the corresponding rows go with them: the strange self-consistency
`n_s(mu_eff_s) - n_s` and the strangeness-equilibrium row `mu_s - mu_d`.

The `cfl` mode **raises** on the flag, the third to do so after `gluons` and
`thermal_neutrinos` and for the same kind of reason: locking pairs the three
flavours at equal densities, so Y_S = +1 identically and no strangeness
fraction is free. The flag keeps both its values in the unpaired modes and is
a statement about the phase in that one.
