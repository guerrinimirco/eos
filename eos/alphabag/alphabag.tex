...
$c_{T_c}$  & \texttt{tc\_coeff} & $0.57 \cdot 2^{1/3} = 0.71815$
                                            & CFL critical temperature as a multiple of $\Delta_0$ \\
\bottomrule
\end{tabular}
\end{center}
%
...
\texttt{Parameters.named('alphabag\_default')} takes it by name. alphaBag ships
...
\texttt{Parameters(alpha=..., B4=...)} names only what changes; the dataclass
is frozen, so \texttt{dataclasses.replace} is how a set already in hand is
modified. \emph{From nuclear-matter parameters:} no route, and none is missing
--- alphaBag has no nuclear sector, so there is no \texttt{nmp.py} and nothing
to invert.
...
else. They vanish identically at $T = 0$. The flag defaults to \texttt{False},
like every other flag of this model: a flag with two legal values is a default
and is off unless asked for, whatever its name, and a bag model without a
thermal gluon gas is the standard MIT configuration rather than a broken one.
Both values are legal in the unpaired modes; in the \texttt{cfl} mode the flag
\emph{raises}, for the reason given in Sec.~\ref{sec:cfl}.
\texttt{gluons} is this model's own
...
\end{cases}
\qquad
T_c = c_{T_c} \Delta_0 , \quad c_{T_c} = 0.57 \cdot 2^{1/3} = 0.71815 ,
\label{eq:gap}
\end{equation}
%
so that $\Delta_0 = 100$~MeV gives $T_c = 71.815$~MeV. Here $c_{T_c}$ is
\texttt{tc\_coeff} of the parameter record rather than a constant of the code:
an inference run over CFL pairing varies it like any other parameter, and the
shipped value is the weak-coupling BCS result with its colour factor. The entropy needs the
...
The gluon term is not part of Eq.~\eqref{eq:Pcfl}, and it is not added to the
totals either: in the \texttt{cfl} mode \texttt{gluons=True} \emph{raises}.
Colour-flavour locking breaks $SU(3)_c \times U(1)_Q$ to a single unbroken
$U(1)_{\tilde Q}$, generated by a mixture of the electric charge and the
eighth colour generator, so of the nine gauge bosons exactly one stays
massless --- the rotated photon, to which the medium is transparent --- while
all eight gluons acquire Meissner masses. The free gas of
Eq.~\eqref{eq:gluons} is $g_g = 2\times 8 = 16$ massless degrees of freedom,
which is the wrong count and the wrong dispersion for this phase; it is
refused here rather than corrected, because a Meissner mass is not the fix.
The light bosonic sector the locked phase actually has is the superfluid
phonon and the pseudo-Goldstone meson octet --- a different sector --- and it
is not implemented (see \texttt{docs/DEFERRED.md}).
...
same $P_\gamma(T)$ the unpaired phase carries. The thermal neutrino gas raises
here too, for a different and non-physical reason --- the paired phase has
never carried it, and the asymmetry is preserved deliberately because closing
it moves published CFL tables.
...
The \texttt{cfl} mode raises on the flag, as it does on \texttt{gluons} and
\texttt{thermal\_neutrinos}, and for the same kind of reason: locking pairs the
three flavours at equal densities, so $Y_S = +1$ identically and no strangeness
fraction is free. The flag keeps both its values in the unpaired modes and is a
statement about the phase in that one.
