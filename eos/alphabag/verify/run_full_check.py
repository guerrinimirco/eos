...
#: The unpaired solver calls in this suite carry every phase-common gas,
#: deliberately: `_matter_only` subtracts one photon gas, one gluon gas and
#: the untracked neutrino flavours off each state before the Euler and
#: free-energy identities are tested, so those gases have to be added and
#: removable rather than woven into the equations.
ALL_GASES = SpeciesFlags(photons=True, gluons=True, thermal_neutrinos=True)

#: The paired phase: the rotated photon is the only massless gauge boson it
#: has, and `solve_cfl` refuses the other two sectors outright.
CFL_BARE = SpeciesFlags()
CFL_GAMMA = SpeciesFlags(photons=True)
    ...
    for n_B in grid:
        ...
        out.append(("trapped",
                    solve_beta_eq_neutrino_trapped(par, n_B, 0.4, T,
                                                   ALL_GASES)))
        out.append(("yc", solve_fixed_yc(par, n_B, 0.0, T, ALL_GASES,
                                         leptons=True)))
        out.append(("yc_nolep", solve_fixed_yc(par, n_B, 0.0, T, ALL_GASES,
                                               leptons=False)))
...
def _check_charge_basis(par, grid, T):
    ...
    worst = 0.0
    for n_B in grid:
        r = solve_beta_eq_neutrinoless(par, n_B, T, ALL_GASES)
...
def _check_mode_closures(par, grid, T):
    ...
    for n_B in grid:
        ...
        r = solve_beta_eq_neutrino_trapped(par, n_B, 0.4, T, ALL_GASES)
        worst = max(worst,
        ...
        r = solve_fixed_yc(par, n_B, 0.0, T, ALL_GASES, leptons=True)
        worst = max(worst, abs(r.Y_C), abs(r.mu_S) / abs(r.mu_B),
                    abs(r.n_B - n_B) / n_B)

        r = solve_fixed_yc(par, n_B, 0.0, T, ALL_GASES, leptons=False)
        # No neutrality here: the phase would be charged at any other Y_C,
        # which is the point.
        worst = max(worst, abs(r.Y_C), abs(r.n_e))

        r = solve_fixed_yc_ys(par, n_B, 0.0, 1.0, T, ALL_GASES)
...
def _check_residual_gate(par, grid, T):
    ...
    for n_B in grid:
        for Delta0 in (50.0, 100.0):
            states.append(("cfl", solve_cfl(par, n_B, T, Delta0, CFL_GAMMA)))
