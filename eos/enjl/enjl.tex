...
All four modes of this repository are closed here, at $T = 0$; what the model
refuses is a temperature, not a mode. And they are not four solvers: a mode is
...
11 & $\nS - Y_S\,\nB^{\rm target}$
     & $\nB^{\rm target}$
     & the held $Y_S$ \hfill (\texttt{fixed\_YC\_YS} only) \\
12 & $n_e + n_{\nu_e} - Y_{L_e}\,\nB^{\rm target}$
     & $\nB^{\rm target}$
...
metastable region beyond, so it may violate $\dd P/\dd\nB \ge 0$. That is real
...
\begin{equation}
  ...
  \label{eq:plateau}
\end{equation}
with $s = 0$ at $T = 0$. The conserved fractions are formed from the
...
$\muB^{\text{co}}$. It also means $c_s^2 = \dd P/\dd\varepsilon = 0$ there
exactly.
