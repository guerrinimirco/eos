...
Eqs.~\eqref{eq:nmed}--\eqref{eq:nsmed}. At $T > 0$ there is no $\kF_i$: $n_i$
...
that is algebra, $\kF_i$ from Eq.~\eqref{eq:nmed} inverted and then
$\nu_i = \sqrt{\kF_i^2 + M_i^2}$. At $T > 0$ it is a genuine numerical
inversion of Eq.~\eqref{eq:nmedT}, done by the shared
\texttt{eos.general.fermi\_integrals.invert\_fermi\_density}.

Where that inversion sits is the structural part. $\kF_i$ depends only on
...
\texttt{nu}         & $\nu_i$, all eight --- the PRIMARY kinetic quantity
                    & Eq.~\eqref{eq:forward} \\
\texttt{kF}         & $\kF_i$, the $T = 0$ derived view of $\nu_i$
                    & $\sqrt{\nu_i^2 - M_i^2}$ \\
