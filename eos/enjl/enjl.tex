...
thermodynamics at $T = 0$ with the cut-off written out, the gap equation in
...
ten-unknown beta-equilibrium residual row by row, and what each of the
repository's four modes means here --- one is closed, three raise, and the
reasons are physics rather than unwritten features. Section~\ref{sec:validation}
quotes what the implementation actually reproduces, measured against the
paper and against the author's own tables. The model is $T = 0$ throughout;
Sec.~\ref{sec:numerics} says where the seam for a finite-temperature
extension is, and what is still open above the first first-order transition.
\end{abstract}
...
\section{Single-species thermodynamics at $T = 0$, and the cut-off}
\label{sec:kinetic}

Each species is a free Fermi gas of degeneracy $g$ and effective mass $M$,
filled to a Fermi momentum $\kF$, and it is the \emph{only} place temperature
would enter. Writing $x = \kF/M$ and $\nu = \sqrt{\kF^2 + M^2}$ for the
...
With $\nu_i = \sqrt{\kF_i^2 + M_i^2}$ the kinetic (effective) potential of
Sec.~\ref{sec:kinetic},
%
\begin{align}
...
\mu_q &= \nu_q + f_q\,\big(g_\omega\omega + \tau_q\, g_\rho\rho\big)
         + \SR_q , &&q = u, d, s ,
\label{eq:muq}\\
\mu_\ell &= \nu_\ell = \sqrt{\kF_\ell^2 + m_\ell^2} ,
         &&\ell = e, \mu .
\label{eq:mul}
\end{align}
%
...
\begin{equation}
\boxed{\;
\varepsilon \;=\; \sum_i \varepsilon(\kF_i, M_i, g_i, \Lambda_i)
  \;+\; 2 G_S \sum_q \big(\nbs_q\big)^2
  \;-\; 4 K\, \nbs_u \nbs_d \nbs_s
...
otherwise. \textbf{No rearrangement term appears in
...
The pressure is the Euler relation at $T = 0$,
%
\begin{equation}
P \;=\; T s + \sum_i \mu_i n_i - \varepsilon
  \;=\; \sum_i \mu_i n_i - \varepsilon ,
\label{eq:P}
\end{equation}
%
and it is how the code computes $P$ --- not by summing Eq.~\eqref{eq:Pmed}
...
them automatically through the $\SR$ inside every $\mu_i$. The entropy density
is $s = 0$ identically, Eq.~\eqref{eq:s}, and hence the free energy density is
$f = \varepsilon - Ts = \varepsilon = -P + \sum_i \mu_i n_i$.
...
\subsection{The mode this model closes}
\label{sec:beta}
...
\texttt{beta\_eq\_neutrinoless} & $(\nB,\ T = 0)$
  & Eqs.~\eqref{eq:beta}--\eqref{eq:neutral} \\
\bottomrule
\end{tabular}
\end{center}
...
\end{equation}
%
and it is longer than the three-unknown gap solve of the fixed-composition
...
\end{cases}
\qquad
\kF_i = \sqrt{\nu_i^2 - M_i^2}\;\;(\nu_i > M_i),\;\; 0 \text{ otherwise},
\label{eq:forward}
\end{equation}
%
with $\mu_i$ from Eq.~\eqref{eq:beta}, and then $n_i$, $n^s_i$, $J_\omega$,
$J_\rho$ and the two $\SR$ expressions from Secs.~\ref{sec:kinetic}
and~\ref{sec:meanfield}. The ten residuals, in the order the code assembles
them, are
...
\subsection{The three modes that raise, and why}

Each of the following raises \texttt{NotImplementedError} naming its own
reason rather than returning a state the model does not have:

\begin{center}
\begin{tabular}{p{0.26\textwidth}p{0.66\textwidth}}
\toprule
mode & why it is not reachable \\
\midrule
\texttt{beta\_eq\_neutrino\_trapped}
  & Neutrinos are not among the degrees of freedom of Sec.~\ref{sec:dof}, and
    Eq.~\eqref{eq:beta} carries no lepton-family potential: it is
    $\mu_i = B_i\mu_b - q_i\mu_e$ with $\mu_{\nu_e} = 0$ built in. A trapped
    mode would fix $Y_{L_e}$, and there is no $\mu_{\nu_e}$ for it to fix.
    The mode is meaningful physics for this model and simply not implemented;
    it needs an eleventh unknown and one more row. \\[4pt]
\texttt{fixed\_YC}
  & $Y_C$ is the \emph{non-leptonic} charge fraction of
    Eq.~\eqref{eq:nCdef}, and imposing it replaces the neutrality row~5 by
    $\nC - Y_C \nB = 0$. With \texttt{leptons=False} that is a one-row change
    and $\mu_e$ becomes $-\muC$; with \texttt{leptons=True} the lepton
    potential separates from $\muC$, since neutrality
    $n_e + n_\mu = \nC$ is then an additional condition, and the system gains
    an unknown. Because the repository's flag requires both, the mode raises
    rather than shipping the half of it that is cheap. \\[4pt]
\texttt{fixed\_YC\_YS}
  & The same for $Y_C$, and in addition $\muS = 0$ identically here,
    Eq.~\eqref{eq:basis}: weak equilibrium does not conserve strangeness, so
    the model as closed has no potential conjugate to $\nS$ and $Y_S$ is an
    output. Imposing $Y_S$ requires promoting $\muS$ to an unknown, which
    changes Eq.~\eqref{eq:beta} into the full three-charge projection
    $\mu_i = B_i\muB + C_i\muC + S_i\muS$ for all six strongly interacting
    species. \\
\bottomrule
\end{tabular}
\end{center}
...
\noindent
\textbf{Temperature.} Every expression in Secs.~\ref{sec:kinetic}
and~\ref{sec:assembly} is a $T = 0$ expression, $s = 0$ identically by
Eq.~\eqref{eq:s}, and any $T > 0$ raises. Wherever this repository accepts an
entropy axis in place of a temperature, $s/\nB = 0$ is the only value the
model reaches. \textbf{Species.} The composition of Sec.~\ref{sec:dof} is
fixed by the model: the octet beyond $\Lambda$, the $\Delta$ quartet, a
thermal meson gas, thermal neutrinos and photons are all absent, and
switching one on raises rather than being silently ignored. The muon family
is present and on. A thermal meson gas is not merely unimplemented but
...
\texttt{kF}         & $\kF_i$ & Eq.~\eqref{eq:nmed} inverted \\
...
\texttt{n\_s}       & $n^s_i$, all six strongly interacting species
                    & Eqs.~\eqref{eq:nsmed}, \eqref{eq:nsvac} \\
...
\noindent
The beta-equilibrium entry point returns the same record plus the two
potentials it solved for, $\mu_b$ and $\mu_e$, and the density $\nB$ it was
asked at. \texttt{eos\_response} is not implemented for this model: the
second-derivative quantities of the CompOSE list divide into those that need
$T > 0$ (the heat capacities, the thermal index) and those that need a
conditioning statement the $T = 0$ branch structure of
Sec.~\ref{sec:numerics} has not yet settled.
...
\paragraph{Where finite temperature would enter.} Nowhere except
Sec.~\ref{sec:kinetic}. The couplings are functions of $\nB$, the gap equation
is the same equation with $T$-dependent scalar densities, the rearrangement
terms have the same expressions, and $\varepsilon_0$ is the same number. Only
Eqs.~\eqref{eq:nmed}--\eqref{eq:s} change --- and the vacuum terms
\eqref{eq:nsvac}--\eqref{eq:epsvac} do not change even there, being
$T$-independent by construction. The pressure would revert to the full Euler
form $P = Ts + \sum_i \mu_i n_i - \varepsilon$ of Eq.~\eqref{eq:P}.
...
compositions, densities and parameter sets: the Euler relation and
...
Eq.~\eqref{eq:neutral} to $9\times10^{-13}$~MeV, together with the
