...
\subsection{A second closure: a phase held at fixed $(Y_C, Y_S)$}
\label{sec:coexcomp}

Equations~\eqref{eq:coexmu}--\eqref{eq:coexneutral} close each phase by
neutrality, which is what a \emph{$\beta$-equilibrium} branch pair needs. A
mixed-phase construction consumes the other object: each pure phase carrying a
\emph{held, non-leptonic} charge and strangeness fraction, no leptons at all
and therefore electrically charged, with global neutrality imposed later.
$Y_C = 0.5$, $Y_S = 0$ is symmetric nuclear matter, the mode heavy-ion
comparisons are made in.

There Eq.~\eqref{eq:coexneutral} is replaced, per phase, by
\begin{equation}
  \nC^{(\gamma)} = Y_C\, \nB^{(\gamma)}, \qquad
  \nS^{(\gamma)} = Y_S\, \nB^{(\gamma)}, \qquad
  \gamma \in \{\alpha, \beta\},
  \label{eq:coexcomp}
\end{equation}
two equations in the two unknowns $(\muC^{(\gamma)}, \muS^{(\gamma)})$, and ---
the substantive difference --- Eq.~\eqref{eq:coexmu} is no longer available.
What coexistence equates is the Gibbs free energy per baryon, which Euler at
fixed composition gives as
\begin{equation}
  g = \frac{\varepsilon + P}{\nB} = \muB + Y_C \muC + Y_S \muS .
  \label{eq:gibbs}
\end{equation}
Under the neutral closure the $Y_C \muC$ term cancels against the lepton
contribution to $\varepsilon + P$ --- that is Eq.~\eqref{eq:eulerneutral} ---
and $\muS = 0$, so $g = \muB$ identically and Eq.~\eqref{eq:coexmu} \emph{is}
the chemical condition. At a held $(Y_C, Y_S)$ neither cancellation happens,
each phase carries its own $(\muC, \muS)$, and the two shifts differ. The
conditions are
\begin{align}
  P^{(\alpha)} &= P^{(\beta)} \equiv P^{\text{co}},
  \label{eq:coexPcomp} \\
  g^{(\alpha)} &= g^{(\beta)} \equiv g^{\text{co}},
  \label{eq:coexgcomp}
\end{align}
with no lepton pressures in Eq.~\eqref{eq:coexPcomp} --- the phases carry none
--- and they are two equations in the two unknowns $\muB^{(\alpha)}$ and
$\muB^{(\beta)}$, which are \emph{not} equal. The residual assembled, with
$x = (\muB^{(\alpha)}, \muB^{(\beta)})$ and each phase's own $(\muC, \muS)$
eliminated by Eq.~\eqref{eq:coexcomp} at every evaluation, is
\begin{align}
  r_1(x) &= P^{(\alpha)}(x_1) - P^{(\beta)}(x_2), \nonumber \\
  r_2(x) &= g^{(\alpha)}(x_1) - g^{(\beta)}(x_2),
  \label{eq:coexrescomp}
\end{align}
a two-dimensional root find where the neutral closure needed a bisection in
one. It is seeded by tabulating $(\muB, g, P)$ on a $\muB$ grid for each branch
and reading each branch's $\muB$ and $P$ at a common $g$: $g$ is monotone along
a branch, since Gibbs--Duhem at fixed composition and $T = 0$ gives
$\nB\, \mathrm{d} g = \mathrm{d} P$ with $\nB > 0$, so a sign change of
$P^{(\alpha)} - P^{(\beta)}$ in $g$ is a one-dimensional bracket. That
interpolated crossing is a grid-resolution answer and is the starting point,
not the result.

$\muS$ is dropped from Eq.~\eqref{eq:coexcomp} where a held $Y_S = 0$ at
$T = 0$ leaves its row reading $0 = 0$. Both strangeness carriers of this model
have $S = +1$ (the $\Lambda$ and the $s$ quark) and at $T = 0$ there are no
antiparticles, so every term of $\nS = \sum_i S_i n_i$ is non-negative and
$\nS = 0$ forces each to vanish separately, for any $\muS$ --- the same
statement, and the same remedy, as the empty strangeness row of the mode table
above.

The plateau of Eqs.~\eqref{eq:lever}--\eqref{eq:plateau} is unchanged, except
that $\muB$ joins $\muC$, $\mu_e$ and the effective masses among the quantities
returned as \texttt{nan}: under this closure it is a property of one phase and
not of the mixture. What is uniform across the window is $g^{\text{co}}$, and
Eq.~\eqref{eq:epsneutral} reads
\begin{equation}
  \varepsilon = g^{\text{co}} \nB - P^{\text{co}},
  \label{eq:epscomp}
\end{equation}
which is Eq.~\eqref{eq:gibbs} rearranged and is again exactly linear in $\nB$,
so the plateau needs no further solve here either.

The locator is
\texttt{eos.mixed.construction.enjl\_composition\_coexistences} over
\texttt{eos.mixed.boundaries.locate\_maxwell\_composition}, beside the neutral
pair and for the same layering reason, and the located windows are delivered
through the same \texttt{build\_constructed\_table}. Measured for the shipped
parameters at $Y_C = 0.5$, $Y_S = 0$: $n_{\text{lo}} = 0.34945$ and
$n_{\text{hi}} = 0.47500$~fm$^{-3}$ at $P^{\text{co}} = 22.9282$~MeV/fm$^3$ and
$g^{\text{co}} = 1006.4074$~MeV, with the energy-density crossing of the two
branches at $0.41774$~fm$^{-3}$ inside the window, as it must be.
