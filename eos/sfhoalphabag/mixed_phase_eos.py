...
from eos.alphabag.parameters import AlphaBagParams, get_alphabag_default
...
from eos.general.thermodynamics_leptons import electron_thermo, photon_thermo, neutrino_thermo
from eos.general.physics_constants import hc, hc3
from eos.general.particles import Proton, Neutron
...
class MixedPhaseResult:
    ...
    def Y_S_tot(self) -> float:
        ...
        return self.n_S_tot / self.n_B if self.n_B > 0 else 0.0
...
def solve_eta0_beta(n_B: float, T: float,
    ...
    Equations:
        ...
        12. μ_S_H = 0                                  (strangeness eq in H sector)
    """
...
def solve_eta1_fixed_yc(n_B: float, Y_C: float, T: float,
    ...
    Equations:
        ...
        12.   μ_C_H + μ_eL_H = μ_C_Q + μ_eL_Q       (charge chemical equilibrium)
    """
    if sfho_params is None:
        sfho_params = get_sfho_nucleonic()
    if alphabag_params is None:
    ...
    def equations(x):
        ...
        res[11] = (mu_C_H + mu_eL_H - qua.mu_C - mu_eL_Q) / 100.0   # charge chemical equilibrium
...
def solve_eta1_fixed_chi_yc(T: float, chi: float, Y_C: float,
    ...
    def equations(x):
        ...
        res[11] = (mu_C_H + mu_eL_H - qua.mu_C - mu_eL_Q) / 100.0  # Charge chemical potential
