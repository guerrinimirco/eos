...
\texttt{eos.mixed.scan} moves over $(B^{1/4}, a, m_s)$, and
\texttt{get\_vmit\_custom(B4=..., a=..., m\_s=...)} returns a parametrization
with any subset changed and the rest left at Table~\ref{tab:parameters}.
...
\end{equation}
%
returned under the keys \texttt{cs2\_eq} and \texttt{C\_V}, the latter only at
$T > 0$ where it is defined. Every other freeze of the specification --- frozen
