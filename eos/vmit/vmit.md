...
| `Y_C`, `Y_Le` | Y_C, Y_{L_e} | the fractions, MEASURED on the solved state in every mode -- a mode that fixes one reports what it solved |
...
Accordingly ``eos.vmit`` **raises** on `fixed_YC_YS` with the flag on.
...
|---|---|---|
| `beta_eq_neutrinoless` | `(mu_u, mu_d, mu_s, mu_e, n_u, n_d, n_s)` | `(mu_u, mu_d, mu_e, n_u, n_d)` |
| `beta_eq_neutrino_trapped` | `(mu_u, mu_d, mu_s, mu_e, mu_nue, n_u, n_d, n_s)` | `(mu_u, mu_d, mu_e, mu_nue, n_u, n_d)` |
| `fixed_YC` (leptons) | `(mu_u, mu_d, mu_s, n_u, n_d, n_s, mu_e)` | `(mu_u, mu_d, n_u, n_d, mu_e)` |
