...
expansion --- that is what the model is.
...
\midrule
$\ms$      & \texttt{ms}    & $150$~MeV & strange current quark mass \\
$\Delta$   & \texttt{Delta} & $80$~MeV  & CFL pairing gap, temperature independent here \\
...
\noindent
$\texttt{eos\_response}$ adds $c_s^2$ from Eq.~\eqref{eq:cs2} in closed form.
The heat capacities and the thermal indices of the CompOSE list are not
defined at $T = 0$ and are not returned; the susceptibilities
$\chi_{ab} = \partial n_a/\partial \mu_b$ are singular here, since
Eq.~\eqref{eq:locking} leaves $n_C$ and $n_S$ with no potential to respond to.
