...
from eos.vmit.thermodynamics_quarks import compute_vmit_thermo_from_mu_n
...
def solve_mixed_phase(n_B: float, T: float, eta: float,
    ...
    if eq_mode == "trapped":
        ...
        from eos.zlvmit.trapped_solvers import solve_eta0_trapped
        if abs(eta) < 1e-10:
...
def solve_fixed_chi(T: float, chi: float, eta: float,
    ...
    if eq_mode == "trapped":
        if Y_L is None:
            raise ValueError("Y_L must be provided for trapped mode")
        from eos.zlvmit.trapped_solvers import solve_eta0_fixed_chi_trapped
        if abs(eta) < 1e-10:
