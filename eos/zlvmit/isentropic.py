...
Usage:
    from eos.zlvmit.isentropic import compute_isentropes, create_entropy_interpolator
...
def compute_isentropic_temperature(
    ...
    """
    Find temperature T for given baryon density and target entropy S = s/n_B.
    
    Args:
        n_B: Baryon density (fm⁻³)
        S_target: Target entropy per baryon S = s/n_B
        ...
        T_guess: Initial guess for temperature (if None, uses 0.1 for first point)
    ...
    # Use provided guess or default to 0.1 for first point
    if T_guess is None:
        T_guess = 0.1
...
def compute_isentropes(
    ...
    for S in S_values:
        ...
        T_prev = None  # Store previous temperature as guess
        
        for i, n_B in enumerate(n_B_values):
            # Use previous temperature as guess, or 0.1 for first point
            T_guess = T_prev if i > 0 else 0.1
            
            T = compute_isentropic_temperature(
                n_B, S, interpolator, T_bounds, T_guess=T_guess
            )
            if T is not None:
                T_array[i] = T
                T_prev = T  # Update guess for next point
