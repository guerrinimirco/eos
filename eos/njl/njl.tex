...
$n_{\rm ref}$ & $0.48$ fm$^{-3}$
  & the power law's reference \emph{quark} density,
    i.e.\ $\nB = n_{\rm sat}$ \\
$\lambda = \Lam_{\rm UV}/\Lam$ & $1$
  & the regularization scale; $\lambda \neq 1$ \emph{raises}
...
\texttt{kunkel} & $\eta_D = 1.45$, $\eta_V = 0.7$ \\
...
The \texttt{kunkel} couplings are those of Ref.~\cite{Kunkel2026} and the
regularization is \emph{not}: their calculation is RG-consistent at
$\lambda \simeq 10$ and this one is at $\lambda = 1$, because the counterterm
that makes $\lambda > 1$ finite is not implemented here. The two are not
independent --- RG-consistent gaps run almost $90\%$ above sharp-cutoff ones
--- so the set is a strong-coupling point, not a reproduction of that paper.
