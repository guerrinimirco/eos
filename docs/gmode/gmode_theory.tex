...
% Compile with:  pdflatex gmode_theory && bibtex gmode_theory && pdflatex gmode_theory
...
As $\gamma$ approaches $\omega$ the buoyancy is destroyed and the mode is
suppressed outright, which is what happens in protoneutron stars and merger
remnants at $T\gtrsim$ a few MeV. Using Fermi-surface Urca
rates~\cite{AlfordHarris2018,Alford2020} with the DD2
interaction~\cite{Typel2010} we find $\gamma=\omega$ at $T\simeq5$~MeV for a
1~kHz oscillation, nearly independent of density, consistent
with~\cite{AlfordHarris2019}. Cold stars, $T\ll1$~MeV, are firmly frozen.
...
the eigenfrequency is found by continuing the real root into the lower half
plane, which also fixes the mode's identity, node counting having no meaning for
a complex eigenfunction.
