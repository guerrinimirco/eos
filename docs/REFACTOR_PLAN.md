...
| enjl     | DONE at T=0: section 5 layout, all four modes (mode-as-declaration via general/modes), `thermo_from_mu` (the phase-adapter surface), integrals through general/, verify/, .tex | the eta-family construction over TWO branches of the one functional (multi-window; f(eta) reported), then finite T (the seam is `kinetic_thermo`) | two sessions |
