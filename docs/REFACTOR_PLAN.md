...
| enjl     | DONE at T=0: section 5 layout, spec API, integrals through general/, verify/, .tex; branch rule still open | finite-T extension (cutoff-regularized finite-T integrals — new numerics; the seam is `kinetic_thermo`), the Maxwell/branch rule, the three modes that raise (see DEFERRED) | ~1 week left, in two sessions |
